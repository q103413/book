...
::: info 提示
本文件包含了 `core-site.xml`、`hdfs-site.xml`、`yarn-site.xml` 以及 `mapred-site.xml` 的核心生产配置参数，专供实训参考。
:::

```xml
<!-- hadoop_config_template.conf -->
<configuration>
    <!-- 1. 配置 HDFS 主节点（NameNode）的 RPC 通信地址 -->
    <property>
        <name>fs.defaultFS</name>
        <value>hdfs://master:9000</value>
    </property>

    <!-- [在此处直接粘贴你的 hadoop_config_template.conf 文件的其余配置源码] -->
    # Hadoop集群配置文件模板
    # 请根据实际环境修改以下配置
